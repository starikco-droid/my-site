...
class RsTransitionpackSliderFront extends RevSliderFunctions {
	...
	public function add_addon_files($html, $export){
		...
		$_jsPathMin = file_exists(RS_TRANSITIONPACK_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
		if(!$export->usepcl){
			$export->zip->addFile(RS_PLUGIN_PATH . 'sr6/assets/js/libs/three.min.js', 'js/three.min.js');
			$export->zip->addFile(RS_TRANSITIONPACK_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', 'js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js');
			$export->zip->addFile(RS_TRANSITIONPACK_PLUGIN_PATH . 'sr6/assets/css/revolution.addon.' . $this->pluginTitle . '.css', 'css/revolution.addon.' . $this->pluginTitle . '.css');
		}else{
			$export->pclzip->add(RS_PLUGIN_PATH.'sr6/assets/js/libs/three.min.js', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/js/libs/', PCLZIP_OPT_ADD_PATH, 'js/');
			$export->pclzip->add(RS_TRANSITIONPACK_PLUGIN_PATH.'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', PCLZIP_OPT_REMOVE_PATH, RS_TRANSITIONPACK_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
			$export->pclzip->add(RS_TRANSITIONPACK_PLUGIN_PATH.'sr6/assets/css/revolution.addon.' . $this->pluginTitle . '.css', PCLZIP_OPT_REMOVE_PATH, RS_TRANSITIONPACK_PLUGIN_PATH.'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
		}
		...
		$html = str_replace(RS_PLUGIN_URL.'sr6/assets/js/libs/three.min.js', $export->path_js .'three.min.js', $html);
	...
	private function isEnabled($slider) {
		...
		if($static_slide instanceof RevSliderSlide) $slides[] = $static_slide;
	...
	public function add_scripts() {
		...
		$base = RS_TRANSITIONPACK_PLUGIN_URL . 'sr6/assets/';		
		$_jsPathMin = file_exists(RS_TRANSITIONPACK_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['tpack'] = '<script'. $addition .' src="'. RS_TRANSITIONPACK_PLUGIN_URL . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
	...
	public function write_footer_scripts(){
		echo '<script type="text/javascript">window.RVS ??= {}; window.RVS.ENV ??= {}; window.RVS.ENV.TRANSITIONPACK_URL = "'.RS_TRANSITIONPACK_PLUGIN_URL.'";</script>'."\n";
	}
	...
	public function check_if_ajax_loaded($r, $output) {
		...
		$html = '<link rel="stylesheet" href="'. RS_TRANSITIONPACK_PLUGIN_URL . 'sr6/assets/css/revolution.addon.' . $this->pluginTitle . '.css">'."\n";
