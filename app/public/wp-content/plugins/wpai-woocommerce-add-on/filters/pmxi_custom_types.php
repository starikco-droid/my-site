...
function pmwi_pmxi_custom_types($custom_types) {
    ...
    if (class_exists('WooCommerce')) {
        if (!empty($custom_types['product'])) {
            $custom_types['product']->labels->name = __('WooCommerce Products', 'wpai_woocommerce_addon_plugin');
        }
        if (!empty($custom_types['shop_order'])) {
            $custom_types['shop_order']->labels->name = __('WooCommerce Orders', 'wpai_woocommerce_addon_plugin');
        }
        if (!empty($custom_types['shop_coupon'])) {
            $custom_types['shop_coupon']->labels->name = __('WooCommerce Coupons', 'wpai_woocommerce_addon_plugin');
        }
