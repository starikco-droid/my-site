...
if( ! function_exists( 'woodmart_search_full_screen' ) ) {
	function woodmart_search_full_screen() {
		...
		if ( 'full-screen' === $desktop_search_design || 'full-screen-2' === $desktop_search_design ) {
			...
			$search_args['include_cat_search'] = isset( $settings['search']['include_cat_search'] ) ? $settings['search']['include_cat_search'] : false;
...
if( ! function_exists( 'woodmart_search_form' ) ) {
	function woodmart_search_form( $args = array() ) {
		...
		$include_cat_search = $include_cat_search && 'product' === $post_type ? 'yes' : 'no';
...
if ( ! function_exists( 'woodmart_ajax_suggestions' ) ) {
	function woodmart_ajax_suggestions() {
		...
		if ( isset( $_REQUEST['include_cat_search'] ) && 'yes' === $_REQUEST['include_cat_search'] && 'product' === $post_type ) {
			$product_categories_suggestions = woodmart_get_product_categories_suggestions();
			$suggestions                    = array_merge( $suggestions, $product_categories_suggestions );

			if ( ! empty( $product_categories_suggestions ) ) {
				$suggestions[] = array(
					'value'   => '',
					'divider' => esc_html__( 'Results', 'woodmart' ),
				);
			}
		}
